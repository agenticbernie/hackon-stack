...
function runWith(...items: ReturnType<typeof evidence>[]): RunState {
  return { id: "r", workflowId: "w", objective: "o", workspace: "/tmp", status: "running", createdAt: "", updatedAt: "", stages: { s: { id: "s", status: "running", attempts: 1 } }, evidence: items, selectedContext: [] };
}
...
test("review and metric gates are independently evaluated", () => {
  const review = evidence({ kind: "ReviewEvidence", producer: "reviewer", stageId: "s", source: "review", result: "pass", metadata: { approved: true, blockerCount: 0 } });
