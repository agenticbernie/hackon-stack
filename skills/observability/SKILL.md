---
name: observability
description: Use the observability method with explicit evidence and a next decision.
---

# Observability

Define useful signals, structured logs, traces, dashboards, alerts, and
runbook links around user outcomes and failure modes. Avoid secret leakage
and alert on actionable thresholds with an owner.

## Purpose
Use observability to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns observability, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the observability method to classify facts, assumptions, risks, and alternatives.
