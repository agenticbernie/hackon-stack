...
description: Use the threat modeling method with explicit evidence and a next decision.
---

# Threat modeling

Map assets, actors, trust boundaries, entry points, abuse cases, and impact.
Use STRIDE and least privilege. Convert material threats into mitigations and
adversarial tests with explicit residual risk.

## Purpose
Use threat modeling to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns threat modeling, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the threat modeling method to classify facts, assumptions, risks, and alternatives.
