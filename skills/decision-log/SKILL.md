---
name: decision-log
description: Use the decision log method with explicit evidence and a next decision.
---

# Decision log

Record the decision, date, owner, context, alternatives, evidence, confidence,
and revisit trigger. Distinguish a decision from an assumption and update the
record when new evidence changes the conclusion.

## Purpose
Use decision log to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns decision log, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the decision log method to classify facts, assumptions, risks, and alternatives.
