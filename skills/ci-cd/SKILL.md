---
name: ci-cd
description: Use the ci/cd method with explicit evidence and a next decision.
---

# CI/CD

Make build, test, security, package, and deployment checks deterministic.
Keep credentials scoped, artifacts attributable, failures visible, and
production actions explicitly approved. Do not hide failed required jobs.

## Purpose
Use ci cd to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns ci cd, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the ci cd method to classify facts, assumptions, risks, and alternatives.
