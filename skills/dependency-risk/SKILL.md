...
description: Use the dependency risk method with explicit evidence and a next decision.
---

# Dependency risk

Inventory direct and transitive dependencies, versions, licenses, provenance,
permissions, and known advisories. Prefer minimal trusted changes and record
the verification command and remaining supply-chain exposure.

## Purpose
Use dependency risk to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns dependency risk, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the dependency risk method to classify facts, assumptions, risks, and alternatives.
