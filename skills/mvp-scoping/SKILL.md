---
name: mvp-scoping
description: Use the mvp scoping method with explicit evidence and a next decision.
---

# MVP scoping

Choose the thinnest vertical slice that tests the core risk. Define non-goals,
manual fallbacks, acceptance tests, launch constraints, and the next learning
loop. Do not confuse fewer features with a valid experiment.

## Purpose
Use mvp scoping to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns mvp scoping, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the mvp scoping method to classify facts, assumptions, risks, and alternatives.
