...
description: Use the assumption mapping method with explicit evidence and a next decision.
---

# Assumption mapping

List assumptions by desirability, viability, feasibility, and usability.
Rank each by uncertainty and consequence. Select the cheapest evidence that
could invalidate the riskiest assumption before implementation.

## Purpose
Use assumption mapping to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns assumption mapping, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the assumption mapping method to classify facts, assumptions, risks, and alternatives.
