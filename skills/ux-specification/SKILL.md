...
description: Use the ux specification method with explicit evidence and a next decision.
---

# UX specification

Specify states, transitions, content, empty and error cases, keyboard and
screen-reader behavior, and responsive constraints. Include acceptance checks
that a reviewer can verify without guessing intent.

## Purpose
Use ux specification to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns ux specification, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the ux specification method to classify facts, assumptions, risks, and alternatives.
