...
description: Use the knowledge retrieval method with explicit evidence and a next decision.
---

# Knowledge retrieval

Retrieve only relevant prior learning with source, score, trust, and scope.
State how the retrieved item influenced the current decision and where it did
not apply. Verify old lessons against current evidence.

## Purpose
Use knowledge retrieval to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns knowledge retrieval, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the knowledge retrieval method to classify facts, assumptions, risks, and alternatives.
