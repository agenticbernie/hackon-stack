...
description: Use the analytics instrumentation method with explicit evidence and a next decision.
---

# Analytics instrumentation

Define events from decisions and user outcomes, not vanity counts. Specify
names, properties, privacy limits, ownership, schema versioning, and a
validation query before shipping instrumentation.

## Purpose
Use analytics instrumentation to produce a decision-ready result for the current workflow stage. Keep observations separate from assumptions and recommendations.

## Triggers
Use when the stage objective concerns analytics instrumentation, or when a handoff explicitly requests this capability. Do not invoke it for unrelated implementation work.
...
2. Apply the analytics instrumentation method to classify facts, assumptions, risks, and alternatives.
